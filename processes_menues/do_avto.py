...
def add_avto(root, self):
    ...
    def add():
        ...
        enter_kg.config(highlightbackground="lightgray")

        all_labels = [enter_name.get(), enter_number.get(), enter_kg.get()]
        ...
        if empty_fields:
            ...
            for field_index in empty_fields:
                if field_index == 0:
                    ...
                    enter_kg.config(highlightbackground="red", highlightcolor="red")

            error_lable = Label(popup, text="ПОМИЛКА: Пропущені поля",font=("Arial",12))
            error_lable.place(x=155, y=190)
        ...
        try:
            sql_add = f"INSERT INTO Автотранспорт (\"Назва_транспорту\", \"Номер_транспорту\"," \
                      f"\"Вантажопідйомність\") VALUES" \
                      f" ('{enter_name.get()}', '{enter_number.get()}','{enter_kg.get()}')"
    ...
    ttk.Label(popup, text="Додавання транспорту", font=("Arial", 20, "bold")).pack(pady=3)

    new_ts = Label(popup, text="Назва транспорту:", font=("Arial", 14))
    new_ts.pack(anchor="nw", padx=10, pady=20)
    enter_name = Entry(popup, width=20, font=("Arial", 12))
    enter_name.place(x=210, y=65)

    number_ts = Label(popup, text="Номер транспорту:", font=("Arial", 14))
    number_ts.pack(anchor="nw", padx=10, pady=2)
    enter_number = Entry(popup, width=20, font=("Arial", 12))
    enter_number.place(x=210, y=115)

    kg_ts = Label(popup, text="Вантажопідйомність: ", font=("Arial", 14))
    kg_ts.pack(anchor="nw", padx=10, pady=20)
    enter_kg = Entry(popup, width=20, font=("Arial", 12))
    enter_kg.place(x=210, y=165)
...
def change_avto(root, self):
    ...
    def add():
        ...
        try:
            ...
            sql_update_prod = f"""
                    UPDATE Автотранспорт
                    SET Назва_транспорту = '{enter_name.get()}',
                    Номер_транспорту = '{enter_number.get()}',
                    Вантажопідйомність = '{enter_kg.get()}' 
                    WHERE Код_транспорту = '{self.item_text[0]}'
                """
    ...
    ttk.Label(popup, text="Редагування транспорту", font=("Arial", 20, "bold")).pack()
    ...
    kg_ts.pack(anchor="nw", padx=10, pady=20)
    enter_kg = Entry(popup, width=20, font=("Arial", 12))
    enter_kg.insert(0, f"{self.item_text[3]}")
    enter_kg.place(x=210, y=165)
