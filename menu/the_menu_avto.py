...
from processes_menues.do_avto import add_avto, change_avto
...
def menu_avto(self):
    ...
    for widget in self.avto_frame.winfo_children():
        widget.destroy()
    self.avto_frame.pack(fill='both', expand=True)

    self.cat_name = tk.Label(self.avto_frame, bg="White", bd=0, text="Транспорт", font=("Arial", 25))
    ...
    def delete_all_data():
        try:
            execute_sql_query_insert(conn_str, "DELETE FROM Автотранспорт")
            self.open_avto_menu()
        except:
            delete_error(self.avto_frame)

    start_1 = Button(self.avto_frame,image=self.delete_all,command=delete_all_data)
    ...
    def print_to_pdf():
        ...
        # Заголовки столбцов
        columns = ("№", "Назва", "Номер", "Вантажопідйомність")
    ...
    print_doc = Button(self.avto_frame, image=self.printer, command=print_to_pdf)
    ...
    columns = ("№", "Назва","Номер","Вантажопідйомність")
    tree = ttk.Treeview(self.avto_frame, columns=columns, show="headings")

    def add_avto_():
        add_avto(self.new_window, self)
        self.btn_add_prod.config(state="disabled")

    def delete_th():
        sql_del = f"DELETE FROM Автотранспорт WHERE Код_транспорту = {self.item_text[0]}"
        execute_sql_query_insert(conn_str, sql_del)
        menu_avto(self)

    def add_change_avto():
        self.add_change_window = self.avto_frame
        change_avto(self.add_change_window, self)
        self.change_items.config(state="disabled")

    self.del_items = Button(self.avto_frame, image=self.del_prodmain, command=delete_th)
    self.del_items.config(background="White", highlightbackground="White", activebackground="white")

    self.change_items = Button(self.avto_frame, image=self.change_prodmain, command=add_change_avto)
    ...
    def on_form(event):
        if not entry_find.get():
            entry_find.insert(0, "Назва")  # Вставка текста обратно, если Entry пустой
    ...
    self.avto_frame.bind("<ButtonRelease-1>", on_form)
    ...
    sql_get_prod2 = f"""
            SELECT "Код_транспорту",
            "Назва_транспорту","Номер_транспорту", "Вантажопідйомність" FROM "Автотранспорт";"""
    data = None
    while data is None:
    ...
    def find():
        ...
        sql_find = f"""
             SELECT "Код_транспорту",
            "Назва_транспорту","Номер_транспорту", "Вантажопідйомність" FROM "Автотранспорт"
            WHERE LOWER("Назва_транспорту") LIKE '%{search_term}%';
            """
    ...
    entry_find = Entry(self.avto_frame, width=13, font=("Arial", 12))
    entry_find.insert(0, "Назва")  # Установка значения по умолчанию
    ...
    self.btn_add_prod = Button(self.avto_frame, image=self.add_prodmain,
                               command=add_avto_)
    ...
    ToolTip(self.btn_add_prod, "Додавання транспорту")
