...
from processes_menues.do_order_sell import add_order_sell, change_order_sell
from short_inf_popups import delete_error


def menu_order_sell(self):
    self.frame_menu.pack_forget()
    for widget in self.oreder_sell_menu.winfo_children():
        widget.destroy()
    self.oreder_sell_menu.pack(fill='both', expand=True)
    self.oreder_name = tk.Label(self.oreder_sell_menu, bg="White", bd=0, text="Продаж товару", font=("Arial", 25))
    ...
    if current_sclad:
        ...
        self.sep_name = ttk.Separator(self.oreder_sell_menu)
        self.sep_name.place(x=25 ,y=60 ,anchor="nw", width=120)
        # Додаємо мітку з назвою складу
        self.name_scl = tk.Label(self.oreder_sell_menu, bg="White", bd=0, text=self.chosen_name, font=("Arial", 25))
    ...
    def delete_all_data():
        try:
            execute_sql_query_insert(conn_str, "DELETE FROM Ордер_вибуття")
            self.open_order_sell_menu()
        except Exception:
            delete_error(self.oreder_sell_menu)

    start_1 = Button(self.oreder_sell_menu,image=self.delete_all,command=delete_all_data)
    ...
    def print_to_pdf():
        ...
        for row_id in tree.get_children():
            ...
            pdf.cell(10, 10, str(values[7]), border=1, ln=False)
            pdf.ln()

        # Сохраняем PDF-файл с использованием UTF-8 кодировки
        pdf.output("order_sell.pdf")

    print_doc = Button(self.oreder_sell_menu, image=self.printer, command=print_to_pdf)
    ...
    columns = ("№", "Номер ордеру", "Замовник", "Продукція", "Кількість" ,"Склад", "Дата", "Сума")
    tree = ttk.Treeview(self.oreder_sell_menu, columns=columns, show="headings")

    def open_add_ord_sell():
        self.add_prod_window = self.oreder_sell_menu
        # add_prod(self.add_prod_window, self)
        add_order_sell(self.add_prod_window, self)
        self.btn_add_prod.config(state="disabled")

    def delete_order():
        sql_del = f"DELETE FROM Ордер_вибуття WHERE Код_ордеру_вибуття = {self.item_text[0]}"
        execute_sql_query_insert(conn_str, sql_del)
        self.oreder_sell_menu()

    def add_change_order_sell():
        self.add_change_window = self.oreder_sell_menu
        change_order_sell(self.add_change_window, self)
        self.change_items.config(state="disabled")

    self.del_items = Button(self.oreder_sell_menu, image=self.del_prodmain ,command=delete_order)
    self.del_items.config(background="White", highlightbackground="White", activebackground="white")

    self.change_items = Button(self.oreder_sell_menu, image=self.change_prodmain ,command=add_change_order_sell)
    ...
    def on_form(event):
        ...
        if selected_items:
            tree.selection_remove(tree.selection())
    self.oreder_sell_menu.bind("<ButtonRelease-1>", on_form)
    ...
    sql_get_prod2 = f"""
        SELECT ord."Код_ордеру_вибуття",
        ord."Номер_ордеру_вибуття",
        ...
        ord."Дата",
        ord."Сума"
        FROM public."Ордер_вибуття" AS ord
    ...
    def find():
        ...
        sql_find = f"""
            ...
            INNER JOIN "Продукція" AS p ON ord."Код_продукції" = p."Код_продукції"
            INNER JOIN "Замовник" AS cst ON ord."Код_замовника" = cst."Код_замовника"
            WHERE ord."Код_складу" = {self.current_sclad_id} AND Номер_ордеру_вибуття = {search_term};
        """
    ...
    entry_find = Entry(self.oreder_sell_menu, width=13, font=("Arial", 12))
    ...
    btn_find = Button(self.oreder_sell_menu, image=self.find, command=find)
    ...
    def sort_treeview_column(tree, col, reverse=False):
        ...
        if col in ["Кількість палетів" ,"Номер ордеру" ,"Цех" ,"Код ордеру"]:
    ...
    self.btn_add_prod = Button(self.oreder_sell_menu, image=self.add_prodmain, command=open_add_ord_sell)
    ...
    ToolTip(self.btn_add_prod ,"Додавання продажу")
